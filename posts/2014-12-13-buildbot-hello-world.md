...
Например, с версией `alpha` ассоциирован стенд `http://alpha.my-service.com`. Таким образом, чтобы пересобрать этот стенд, нужно запушить изменения в ветку `alpha` (далее я подразумеваю, что репозиторий у вас уже есть и упомянутые здесь ветки уже созданы).
