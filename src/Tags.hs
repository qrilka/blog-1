...
convertSpecificTagsToLinks :: Tags 
                           -> String 
                           -> Rules ()
convertSpecificTagsToLinks tags aTitle = 
    tagsRules tags $ \tag pattern -> do
        let title = renderHtml $ H.preEscapedToHtml $ aTitle ++ " <span class=\"tag-in-title\">" ++ tag ++ "</span>"
        route idRoute
        compile $ do
...
convertTagsToLinks = do
    tags <- ask
    lift $ convertSpecificTagsToLinks tags "Все статьи по теме"
    return ()
